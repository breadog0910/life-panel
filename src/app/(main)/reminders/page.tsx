          ...
          {reminders.length === 0 ? (
            <div className="bg-white rounded-card border border-[#e3f2fd] p-5">
              <div className="text-center py-12 text-sm text-[#90a4ae]">
                🔔 还没有设置提醒
                <br />
                <span className="text-xs">点击「新建提醒」添加</span>
              </div>
            </div>
          ) : (
            <div className="space-y-2">
              {reminders.map((r) => {
                const rl = REPEAT_LABELS[r.repeat_rule];
                return (
                  <div
                    key={r.id}
                    className={`bg-white rounded-card border p-4 flex items-center gap-3 transition-all ${
                      r.enabled
                        ? "border-[#e3f2fd]"
                        : "border-[#e3f2fd]/50 opacity-60"
                    }`}
                  >
                    {/* Toggle switch */}
                    <button
        ...
        <div>
          {editing ? (
            <div className="bg-white rounded-card border border-[#e3f2fd] p-5 space-y-4">
              ...
              <div>
                <label className="text-xs text-[#90a4ae] mb-1.5 block">提醒内容</label>
                <input
                  value={title}
              ...
              <div>
                <label className="text-xs text-[#90a4ae] mb-1.5 block">提醒时间</label>
                <input
                  type="time"
                  value={triggerTime}
              ...
              <div>
                <label className="text-xs text-[#90a4ae] mb-1.5 block">重复规则</label>
                <div className="flex gap-2">
                  {(["daily", "weekly", "none"] as Reminder["repeat_rule"][]).map((rule) => {
                    const rl = REPEAT_LABELS[rule];
                    return (
                      <button
                        key={rule}
              ...
              {editId && (
                <div className="flex items-center justify-between">
                  <span className="text-xs text-[#90a4ae]">启用</span>
                  <button
                    onClick={() => setEnabled(!enabled)}
                    className={`relative w-10 h-6 rounded-full transition-colors ${
                      enabled ? "bg-[#42a5f5]" : "bg-[#cfd8dc]"
                    }`}
                  >
                    <span
                      className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${
              ...
              {editId && (
                ...
                >
                  <Trash2 className="size-3.5" /> 删除提醒
                </button>
              )}
            </div>
          ) : (
